/****************************************************************************************************************
SP 명  : PROC_TB_DW_DIM_ZDWT005_COOIS
SP Owner    : scm
SP 제목  : INPUT 
최초작성자 : 최은수
최초작성일 : 2024.09.11
SP 실행 예 : EXEC scm.PROC_TB_DW_DIM_ZDWT005_COOIS
SP 설명  : PLAN 입력
변경이력  : 변경일/변경자/요청자/요청일/내용 
******************************************************************************************************************/
CREATE PROCEDURE [scm].[PROC_TB_DW_DIM_ZDWT005_COOIS]
 @CUR_DATE DATE
AS
BEGIN
DECLARE 
  ...
  @TABLE_NM VARCHAR(50) = 'TB_DW_DIM_ZDWT005_COOIS',   -- TARGET TABLE NAME
  @SP_START_EXEC DATETIME = GETDATE()             -- SP 실행시작시간
 BEGIN TRY
DELETE FROM scm.TB_DW_DIM_ZDWT005_COOIS
WHERE ERSDA IN (SELECT DISTINCT ERSDA FROM ODS_YG1.ODS_SCM.TB_ODS_ZDWT005_COOIS_CHANGE)
SET @DELETE_CNT=@@ROWCOUNT
INSERT INTO DW_YG1.scm.TB_DW_DIM_ZDWT005_COOIS
SELECT *
  ,@CUR_DATE AS LOAD_DT
FROM ODS_YG1.ods_scm.TB_ODS_ZDWT005_COOIS_CHANGE
