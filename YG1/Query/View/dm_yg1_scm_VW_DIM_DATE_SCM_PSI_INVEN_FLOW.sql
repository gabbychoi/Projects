CREATE     VIEW [dm_yg1_scm].[VW_DIM_DATE_SCM_PSI_INVEN_FLOW] AS 
 WITH LIST AS (
  SELECT DISTINCT ERSDA AS SAP_LOAD_DATE
    , ERSDA AS SLICER_DATE
    FROM dm_yg1_scm.FACT_PSI_INVEN_FLOW_TB
   WHERE [ERSDA] >= DATEADD(MONTH, -3, GETDATE())
 ),
 DIM_D AS (
